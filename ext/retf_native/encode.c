...
static VALUE retf_encode_object(VALUE self, VALUE str_buffer);
static int encode_hash_pair(VALUE key, VALUE value, VALUE buffer);
static VALUE retf_compress_data(VALUE str_buffer);
static VALUE retf_encode_term(VALUE term, VALUE str_buffer);
...
VALUE retf_encode(VALUE self, VALUE to_encode, VALUE compress)
{
    ...
    if (RTEST(compress))
    {
        retf_encode_term(to_encode, str_buffer);
        return retf_compress_data(str_buffer);
    }
...
static void retf_ensure_str_extra_capacity(VALUE str, size_t required)
{
...
static VALUE retf_compress_data(VALUE str_buffer)
{
    ...
    {
        rb_raise(rb_eArgError, "encoded data is too large length must fit in a 32-bit unsigned integer");
    }

    uint32_t nlen = htobe32(len);

    // compress the data
    VALUE zipped_str = rb_funcall(retf_constants_get_zlib_deflate(), rb_intern("deflate"), 1, str_buffer);
...
static VALUE retf_encode_term(VALUE term, VALUE str_buffer)
{
    ...
    {
    case T_NIL:
        return rb_str_cat(str_buffer, "w\003nil", 6);
    case T_TRUE:
        return rb_str_cat(str_buffer, "w\004true", 7);
    case T_FALSE:
        return rb_str_cat(str_buffer, "w\005false", 8);
    case T_FIXNUM:
    case T_BIGNUM:
        return retf_encode_integer(term, str_buffer);
    case T_FLOAT:
        return retf_encode_float(term, str_buffer);
    case T_STRING:
        return retf_encode_string(term, str_buffer);
    case T_ARRAY:
        return retf_encode_array(term, str_buffer);
    case T_HASH:
        return retf_encode_map(term, str_buffer);
    case T_SYMBOL:
        return retf_encode_atom(term, str_buffer);
    case T_CLASS:
    case T_MODULE:
        return retf_encode_class(term, str_buffer);
    case T_OBJECT:
        return retf_encode_object(term, str_buffer);
    default:
...
VALUE retf_encode_integer(VALUE self, VALUE str_buffer)
{
    ...
    {
        return encode_fixed_integer(RB_FIX2LONG(self), str_buffer);
    }
...
VALUE retf_encode_string(VALUE self, VALUE str_buffer)
{
    ...
    if (len > RETF_USIZE_MAX)
    {
        rb_raise(rb_eArgError, "string is too long to encode, bytesize must fit in a 32-bit unsigned integer");
    }

    retf_ensure_str_extra_capacity(str_buffer, 5 + len);
...
VALUE retf_encode_array(VALUE self, VALUE str_buffer)
{
    ...
    if (len > RETF_USIZE_MAX)
    {
        rb_raise(rb_eArgError, "array is too long to encode, length must fit in a 32-bit unsigned integer");
    }
    ...
    for (long i = 0; i < len; i++)
    {
        ...
        retf_encode_term(elem, str_buffer);
    }
...
VALUE retf_encode_map(VALUE self, VALUE str_buffer)
{
    ...
    if (size > RETF_USIZE_MAX)
    {
        rb_raise(
            rb_eArgError,
            "map is too large to encode, size must fit in a 32-bit unsigned integer");
    }

    retf_ensure_str_extra_capacity(str_buffer, 5 + (size * 2));
...
static int encode_hash_pair(VALUE key, VALUE value, VALUE buffer)
{
    retf_encode_term(key, buffer);
    retf_encode_term(value, buffer);
    return 0;
}
...
static VALUE retf_encode_object(VALUE self, VALUE str_buffer)
{
    ...
    VALUE etf = rb_funcall(self, as_etf_sym, 0);

    Check_Type(etf, T_HASH);
    ...
    retf_encode_class(class, str_buffer);
...
VALUE retf_encode_atom(VALUE self, VALUE str_buffer)
{
    ...
    {
        rb_raise(rb_eArgError, "atom is too long to encode, length must fit in a 32-bit unsigned integer");
    }
    ...
    {
        char new_len = len;
        retf_ensure_str_extra_capacity(str_buffer, 2 + len);
    ...
    {
        retf_ensure_str_extra_capacity(str_buffer, 3 + len);
...
VALUE retf_encode_class(VALUE self, VALUE str_buffer)
{
    ...
    VALUE elixirized = rb_funcall(name, rb_intern("gsub"), 2, rb_str_new_lit("::"), rb_str_new_lit("."));

    // 7 is the length of "Elixir."
    if (7 + rb_str_strlen(elixirized) > 255)
    {
        rb_raise(rb_eArgError, "class name is too long to encode, must be no greater than 255 characters");
    }
